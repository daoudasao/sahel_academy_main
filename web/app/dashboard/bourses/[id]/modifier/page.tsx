...
export default function ModifierBoursePage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  const router = useRouter();
  const [loading, setLoading] = useState(true);
  const [notFound, setNotFound] = useState(false);
  const [loadedChampIds, setLoadedChampIds] = useState<string[]>([]);
  ...
  const handleDelete = async () => {
    setDeleting(true);
    try {
      await boursesApi.remove(id);
      router.push("/dashboard/bourses");
    } catch (err) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const [b, depts] = await Promise.all([boursesApi.get(id), departementsApi.list()]);
        ...
        setForm({
          ...
          dateLimite: b.dateLimite ? String(b.dateLimite).slice(0, 10) : "",
        });
        setImageUrl(b.imageUrl ?? "");
        setDocumentAdmissionUrl(b.documentAdmissionUrl ?? "");
        setDocumentAdmissionNom(b.documentAdmissionNom ?? "");
        setMessageAdmission(b.messageAdmission ?? "");
        const cs: ChampFormulaire[] = (b.champs ?? []).map((c: any) => ({
          id: c.id,
          label: c.label,
          type: c.type,
          obligatoire: c.obligatoire,
          options: c.options ?? [],
          aide: c.aide ?? undefined,
          cle: c.cle ?? undefined,
        }));
        setChamps(cs);
        setLoadedChampIds(cs.map((c) => c.id));
      } catch {
        setNotFound(true);
      } finally {
        setLoading(false);
  ...
  const updateChamp = (cid: string, updates: Partial<ChampFormulaire>) => {
    setChamps((prev) => prev.map((c) => (c.id === cid ? { ...c, ...updates } : c)));
  };

  const removeChamp = (cid: string) => {
    setChamps((prev) => prev.filter((c) => c.id !== cid));
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await boursesApi.update(id, {
        ...
        dateLimite: form.dateLimite ? new Date(form.dateLimite).toISOString() : undefined,
      });
      // Réconcilie les champs : supprime les anciens puis recrée l'état courant
      await Promise.all(loadedChampIds.map((cid) => boursesApi.removeChamp(id, cid).catch(() => {})));
      for (let i = 0; i < champs.length; i++) {
        const c = champs[i];
        await boursesApi.addChamp(id, {
          label: c.label,
          type: c.type,
          obligatoire: c.obligatoire,
          options: c.options,
      ...
      setSaved(true);
      setTimeout(() => router.push("/dashboard/bourses"), 800);
    } catch (err) {
    ...
    return (
      <>
        <div className="page-header">
          <h1 className="page-title">Bourse introuvable</h1>
        </div>
        <div className="page-body">
          <InfoBanner>
            <strong>Objectif :</strong> Éditer une offre de bourse et ajuster les champs de son formulaire.<br />
            <strong>Utilité :</strong> Permet d&apos;affiner les critères de sélection ou de corriger les informations requises pour les futures candidatures.
          </InfoBanner>
          <div className="empty-state">
            <h3>Aucune bourse trouvée avec cet identifiant.</h3>
            <Link href="/dashboard/bourses" className="btn btn-primary" style={{ marginTop: 16 }}>
              ← Retour aux bourses
            </Link>
          </div>
        </div>
    ...
    <div className="w-full max-w-full space-y-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="space-y-0.5">
              <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-emerald-500/20 text-emerald-300 text-[11px] font-semibold border border-emerald-500/30">
                <span>Édition de Bourse</span>
              </div>
              <h1 className="text-xl sm:text-2xl font-bold tracking-tight text-white">
                Modifier la Bourse
              </h1>
              <p className="text-xs text-emerald-100/75">
                Ajustez les modalités et le formulaire de candidature.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="page-body">
        <form onSubmit={handleSubmit}>
          ...
          <div className="card animate-in" style={{ padding: 28, marginBottom: 24 }}>
            ...
            <div className="form-grid">
              <div className="form-group" style={{ gridColumn: "1 / -1" }}>
                ...
                <input className="form-input" type="text" value={form.titre} onChange={(e) => handleFormChange("titre", e.target.value)} required />
              </div>
              <div className="form-group" style={{ gridColumn: "1 / -1" }}>
                <label className="form-label">Description</label>
                <textarea className="form-textarea" value={form.description} onChange={(e) => handleFormChange("description", e.target.value)} />
              </div>
          ...
          <div className="card animate-in" style={{ padding: 28, marginBottom: 24 }}>
            ...
            <p style={{ fontSize: 13, color: "var(--text-muted)", margin: "0 0 20px" }}>
              Ces éléments seront automatiquement envoyés dans le chat support du candidat retenu lors de la validation de son admission.
            </p>
            ...
            <div className="form-grid">
              ...
              <div className="form-group" style={{ gridColumn: "1 / -1" }}>
                ...
                <p style={{ fontSize: 12, color: "var(--text-muted)", margin: "0 0 8px" }}>
                  Ce fichier (PDF, DOC, Image) sera envoyé dans le chat support de l&apos;étudiant admis.
                </p>
              ...
              <div className="form-group" style={{ gridColumn: "1 / -1" }}>
                ...
                <p style={{ fontSize: 12, color: "var(--text-muted)", margin: "0 0 8px" }}>
                  Rédigez le texte envoyé dans le chat support de l&apos;étudiant. Les variables ci-dessous utilisent le nom du compte de l&apos;utilisateur.
                </p>
                    ...
                    <>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div className="form-group">
                          ...
                          <input className="form-input" type="text" placeholder="ex: Nom complet" value={champ.label} onChange={(e) => updateChamp(champ.id, { label: e.target.value })} />
                        </div>
                        <div className="form-group">
                          <label className="form-label" style={{ fontSize: 12 }}>Texte d&apos;aide (optionnel)</label>
                          <input className="form-input" type="text" placeholder="ex: Entrez votre nom et prénom" value={champ.aide || ""} onChange={(e) => updateChamp(champ.id, { aide: e.target.value || undefined })} />
                        </div>
                      </div>

                      <div style={{ display: "flex", alignItems: "center", gap: 16, marginTop: 12 }}>
                        <label style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 13, cursor: "pointer" }}>
                          <input type="checkbox" checked={champ.obligatoire} onChange={(e) => updateChamp(champ.id, { obligatoire: e.target.checked })} style={{ accentColor: "var(--accent-primary)" }} />
                          Obligatoire
                        </label>
                      </div>
                        ...
                        <div style={{ marginTop: 16, paddingTop: 16, borderTop: "1px solid var(--border-color)" }}>
                          ...
                          {champ.options.map((opt, optIndex) => (
                            <div key={optIndex} style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 8 }}>
                              ...
                              <input className="form-input" type="text" value={opt} onChange={(e) => updateOption(champ.id, optIndex, e.target.value)} style={{ flex: 1 }} />
                ...
                <div className="card" style={{ padding: 20 }}>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-7 gap-2.5">
                    {(Object.keys(typeLabels) as TypeChamp[]).map((type) => (
                      <button key={type} type="button" onClick={() => addChamp(type)}
                        style={{ padding: "16px 12px", border: "1px solid var(--border-color)", borderRadius: "var(--radius-md)", background: "var(--bg-card)", cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: 8, transition: "all 150ms", fontFamily: "inherit" }}
                        onMouseOver={(e) => { e.currentTarget.style.borderColor = "var(--accent-primary)"; e.currentTarget.style.background = "rgba(20,184,166,0.05)"; }}
          ...
          {saved && (
            <div style={{ marginTop: 16, padding: "12px 16px", borderRadius: "var(--radius-md)", background: "rgba(16,185,129,0.1)", color: "#059669", fontWeight: 600, fontSize: 13 }}>
              ✓ Bourse modifiée avec succès !
            </div>
          )}
        </form>
        ...
        <Dialog
          ...
          onOpenChange={(open) => {
            if (!open && !deleting) setShowDeleteModal(false);
          }}
        >
          <DialogContent className="max-w-md">
            <DialogHeader>
              ...
              <DialogDescription className="text-xs text-slate-500 leading-relaxed pt-1">
                Êtes-vous sûr de vouloir supprimer définitivement l&apos;offre &quot;<strong>{form.titre || "cette bourse"}</strong>&quot; ?
              </DialogDescription>
            </DialogHeader>
            ...
            <div className="p-3 bg-rose-50/70 border border-rose-200/80 rounded-xl text-xs text-rose-800 space-y-1 my-2">
              ...
              <p className="text-[11px] text-rose-700 leading-normal">
                Toutes les candidatures et la configuration des champs associées seront définitivement supprimées.
              </p>
            </div>
